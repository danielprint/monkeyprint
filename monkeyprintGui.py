...
#        slicerListenerId = GLib.timeout_add(100, self.modelCollection.checkSlicerThreads)
        # Update the progress bar, projector image and 3d view. during prints.
        pollPrintQueuesId = GLib.timeout_add(50, self.pollPrintQueues)
...
#DO THIS LATER ON PRINT BUTTON PRESS!
        ...
        if self.runningOnRasPi or self.programSettings['printOnRaspberry'].value:
            GLib.io_add_watch(self.socket.fileDescriptor, GLib.IO_IN, self.socket.callbackIOActivity, self.socket.socket)
        if not self.runningOnRasPi and self.programSettings['printOnRaspberry'].value:
            # Connection poll.
            GLib.timeout_add(500, self.socket.pollRasPiConnection)
            # Connection timeout counter.
            GLib.timeout_add(1000, self.socket.countdownRasPiConnection)
        ...
        sliceCombinerListenerId = GLib.timeout_add(100, self.modelCollection.checkSliceCombinerThread)
        # Update the progress bar, projector image and 3d view. during prints.
        pollPrintQueuesId = GLib.timeout_add(50, self.pollPrintQueues)
        # Request status info from raspberry pi.
    #    pollRasPiConnectionId = GLib.timeout_add(500, self.pollRasPiConnection)
        # Request status info from slicer.
        pollSlicerStatusId = GLib.timeout_add(100, self.pollSlicerStatus)
        ...
        if filename != None:
            printjobLoadFunctionId = GLib.idle_add(self.loadPrintjob, filename)
    ...
    def callbackOpen(self, event):
        ...
        dialog = Gtk.FileChooserDialog("Load project", None, Gtk.FileChooserAction.OPEN, ('Cancel', Gtk.ResponseType.CANCEL, 'Open', Gtk.ResponseType.OK))
    ...
    def callbackSaveProject(self, event):
        ...
        filepath = ""
        # File open dialog to retrive file name and file path.
        dialog = Gtk.FileChooserDialog("Save project", None, Gtk.FileChooserAction.SAVE, ('Cancel', Gtk.ResponseType.CANCEL, 'Save', Gtk.ResponseType.OK))
        dialog.set_modal(True)
        dialog.set_default_response(Gtk.ResponseType.OK)
        dialog.set_current_folder(self.programSettings['currentFolder'].value)
        # File filter for the dialog.
...
#            return True # returning True avoids it to signal "destroy-event"
    ...
    #    listenerSliceSlider = GLib.timeout_add(100, self.sliceSlider.updateImage)
...
#TODO        self.checkButtonCameraTrigger1 = monkeyprintGuiHelper.toggleButton(string="During exposure",)
    ...
    def callbackSaveSlices(self, widget, data=None):
        # File open dialog to retrive file name and file path.
        dialog = Gtk.FileChooserDialog("Save slices", None, Gtk.FileChooserAction.SAVE, ('Cancel', Gtk.ResponseType.CANCEL, 'Save', Gtk.ResponseType.OK))
        dialog.set_modal(True)
        dialog.set_default_response(Gtk.ResponseType.OK)
        dialog.set_current_folder(self.programSettings['currentFolder'].value)
        # File filter for the dialog.
        ...
        if response == Gtk.ResponseType.OK:
            ...
            infoWindow.set_type_hint(Gdk.WindowTypeHint.DIALOG)
            infoWindow.show()
            infoBox = Gtk.VBox()
...
#        self.viewModels.set_headers_visible(False)    # Make column invisible.
        ...
        self.modelSelection.set_mode(Gtk.SelectionMode.SINGLE)
    ...
    def callbackLoad(self, widget, data=None):
        ...
        # File open dialog to retrive file name and file path.
        dialog = Gtk.FileChooserDialog("Load model", None, Gtk.FileChooserAction.OPEN, ('Cancel', Gtk.ResponseType.CANCEL, 'Open', Gtk.ResponseType.OK))
        dialog.set_default_response(Gtk.ResponseType.OK)
...
class dialogFirmware(Gtk.Window):
    ...
    def callbackButtonPath(self, widget, data=None):
        ...
        filepath = ""
        # File open dialog to retrive file name and file path.
        dialog = Gtk.FileChooserDialog("Choose firmware file", None, Gtk.FileChooserAction.OPEN, ('Cancel', Gtk.ResponseType.CANCEL, 'Open', Gtk.ResponseType.OK))
        dialog.set_modal(True)
        dialog.set_default_response(Gtk.ResponseType.OK)
    ...
    def callbackFlash(self, widget, data=None):
        ...
        listenerIdAvrdude = GLib.timeout_add(100, self.listenerAvrdudeThread)
...
class dialogManualControl(Gtk.Window):
    ...
    def callbackButtonSend(self, widget, data=None):
        ...
        # Add avrdude thread listener to gui main loop.
        listenerIdSerial = GLib.timeout_add(100, self.listenerSerialThread)
...
class dialogSettings(Gtk.Window):
    ...
    def createMainSettingsTab(self):
        ...
        self.radioButtonRaspiOff.set_tooltip_text("Use this option if you want print jobs to run directly from your PC. Your PC has to remain connected to the printer during prints.")
        ...
        self.radioButtonRaspiOn.set_tooltip_text("Use this option if you want print jobs to run from a Raspberry Pi. You can remove your PC during prints.")
        ...
        self.radioButtonMonkeyprintBoardOn.set_tooltip_text("Use this option if you want to use the Monkeyprint board for controlling your hardware.")
        ...
        self.radioButtonMonkeyprintBoardOff.set_tooltip_text("Use this option if you want to use a G-Code based board for controlling your hardware.")
...
#        self.buttonSerialPing.show()
    ...
    def callbackSerialTest(self, widget, data=None):
        ...
        listenerIdSerial = GLib.timeout_add(500, self.listenerSerialThread)
...
class modulesListView(Gtk.HBox):
    def __init__(self, settings, parentWindow, console=None):
        ...
        self.scrolledWindowPrintProcess.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.ALWAYS)
    ...
    #    self.columnValuePrintProcess.set_sort_column_id(3)
        ...
        # Avoid multiple selection.
        self.printProcessModuleSelection.set_mode(Gtk.SelectionMode.SINGLE)
        # Connect to selection change event function.
